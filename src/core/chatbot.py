...
class TranslationChatbot:
    ...
    def _parse_intent(self, message: str) -> str:
        ...
        if any(keyword in message_lower for keyword in ["thay đổi", "sửa", "change", "replace"]):
            return "change_translation"
        elif any(keyword in message_lower for keyword in ["tìm", "tại sao", "why", "find", "search"]):
            return "query_translation"
        elif any(keyword in message_lower for keyword in ["hàng loạt", "batch", "all", "tất cả"]):
            return "batch_changes"
        elif any(keyword in message_lower for keyword in ["áp dụng", "apply", "save", "lưu"]):
    ...
    async def _handle_translation_query(self, message: str, context: Dict[str, Any]) -> str:
        ...
        for source, target in self.current_translations.items():
            if any(term in source.lower() or term in target.lower() for term in message.lower().split()):

...
class TranslationEngine:
    ...
    def __init__(self, openai_api_key: str = None, anthropic_api_key: str = None):
        ...
        self.anthropic_engine = AnthropicEngine(anthropic_api_key) if anthropic_api_key else None
    ...
    async def translate_text(self, text: str, target_lang: SupportedLanguage, 
        ...
        # Check cache first
        cache_key = f"{text}_{target_lang.value}_{source_lang.value if source_lang else 'auto'}"
        ...
        try:
            ...
            if self.openai_engine:
                translated = await self.openai_engine.translate_text(text, target_lang, source_lang, custom_dict, context)
            elif self.anthropic_engine:
                translated = await self.anthropic_engine.translate_text(text, target_lang, source_lang, custom_dict, context)
            else:
                # Fallback to Google Translate
                translated = await self.google_engine.translate_text(text, target_lang, source_lang, custom_dict, context)
            ...
            # Fallback to Google Translate
            return await self.google_engine.translate_text(text, target_lang, source_lang, custom_dict, context)

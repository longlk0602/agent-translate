...
class DictionaryManager:
    ...
    def extract_keywords_from_reference(self, reference_files: List[str]) -> Dict[str, str]:
        ...
        for i in range(0, len(lines) - 1, 2):
            if i + 1 < len(lines):
                ...
                if source_line and target_line:
                    ...
                    if abs(len(source_line) - len(target_line)) < max(len(source_line), len(target_line)) * 0.5:

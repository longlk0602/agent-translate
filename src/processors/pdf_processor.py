...
class PDFProcessor(BaseDocumentProcessor):
    ...
    def reconstruct_with_reportlab(self, original_path: str, translated_content: Dict[str, Any], output_path: str) -> str:
    ...
    def apply_translations(self, extracted_content: Dict[str, Any], translations: List[str]) -> Dict[str, Any]:
        ...
        for page_data in translated_content['pages']:
            for block_data in page_data['text_blocks']:
                for line_data in block_data['lines']:
                    if line_data['text'].strip() and translation_idx < len(translations):
                        line_data['text'] = translations[translation_idx]
                        translation_idx += 1
        
        return translated_content
    
    def process_with_font_fallback(self, original_path: str, translated_content: Dict[str, Any], output_path: str) -> str:
        """Xử lý với fallback font strategy"""
        try:
            # Thử phương pháp 1: PyMuPDF với font Unicode
            return self.reconstruct_document(original_path, translated_content, output_path)
        except Exception as e:
            print(f"Phương pháp 1 thất bại: {e}")
            try:
                # Thử phương pháp 2: ReportLab
                return self.reconstruct_with_reportlab(original_path, translated_content, output_path)
    ...
    def create_text_only_pdf(self, translated_content: Dict[str, Any], output_path: str) -> str: 

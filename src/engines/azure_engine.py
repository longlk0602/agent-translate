        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, params=params, headers=headers, json=body) as response:
                if response.status == 200:
                    ...
                    raise Exception(f"Azure Translator API error {response.status}: {error_text}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, params=params, headers=headers, json=body) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise Exception(f"Azure Language Detection API error {response.status}: {error_text}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise Exception(f"Azure Languages API error {response.status}: {error_text}")

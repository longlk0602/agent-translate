...
class TranslationAgent:
    ...
    async def translate_document(self, request: TranslationRequest) -> TranslationResult:
        ...
        try:
            ...
            if request.reference_files:
                ...
                reference_keywords = self.dictionary_manager.extract_keywords_from_reference(request.reference_files)
            ...
            output_path = self._generate_output_path(request.source_file_path, request.target_language)
            ...
            self.chatbot.current_translations = self._extract_translation_pairs(content, translated_content)
    ...
    async def _translate_document_content(self, content: Dict[str, Any], 
                                        target_lang: SupportedLanguage,
                                        source_lang: Optional[SupportedLanguage] = None,
                                        custom_dict: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        if 'text' in content:
            ...
            for sheet in content['sheets']:
                for row in sheet['cells']:
                    for cell in row:
                        if cell['value'].strip():
                            cell['value'] = await self.translation_engine.translate_text(
                                cell['value'], target_lang, source_lang, custom_dict
                            )
        
        return content
    
    def _generate_output_path(self, source_path: str, target_lang: SupportedLanguage) -> str:
    ...
    def _extract_translation_pairs(self, original_content: Dict[str, Any], 
                                 translated_content: Dict[str, Any]) -> Dict[str, str]:
        ...
        if 'text' in original_content:
            ...
            for orig_para, trans_para in zip(original_content['paragraphs'], translated_content['paragraphs']):
                pairs[orig_para['text']] = trans_para['text']
        elif 'pages' in original_content:
            for orig_page, trans_page in zip(original_content['pages'], translated_content['pages']):
                pairs[orig_page['text']] = trans_page['text']
        elif 'slides' in original_content:
            for orig_slide, trans_slide in zip(original_content['slides'], translated_content['slides']):
                for orig_frame, trans_frame in zip(orig_slide['text_frames'], trans_slide['text_frames']):

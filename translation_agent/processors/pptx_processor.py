                    ...
                    for row_idx, row in enumerate(shape.table.rows):
                        ...
                        for cell_idx, cell in enumerate(row.cells):
                            ...
                            if cell_text:
                                ...
                                row_data.append({'cell_index': cell_idx, 'text': '', 'original_text': ''})
                        table_data.append({'row_index': row_idx, 'cells': row_data})
                    shape_data['table_data'] = table_data
                
                # Only add shapes that have text content
                if (shape_data['text_content'] or 
                    shape_data['text_frames'] or 
                    shape_data.get('table_data')):
    ...
    def apply_translations(self, extracted_content: Dict[str, Any], translations: List[str]) -> Dict[str, Any]:
        ...
        for slide_data in translated_content['slides']:
            for shape_data in slide_data['shapes']:
                ...
                if shape_data['text_frames']:
                    for frame_data in shape_data['text_frames']:
                        if frame_data['text'].strip() and translation_idx < len(translations):
